    ...
    for (let [index, str] of array.entries() ) {  
       if (str.match(/([12]\d{3}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01]))/g)) {
           if( array[index+1].match(/(([0-1]?[0-9])|(2[0-3]))[0-5][0-9][0-5][0-9]/g) ) {   
                return (array[index].substr(0,4) +'-'+ array[index].substr(4,2) +'-'+ array[index].substr(6,2) + ' ' + array[index+1].substr(0,2) +':'+ array[index+1].substr(2,2) +':'+ array[index+1].substr(4,2) ) //+'.000Z'
           } else { 
    ...
    try {
        ...
        if (ERROR.length>2) { // if this array contains more than 2 lines "ERROR BEGIN" and "ERROR END"   
            ...
            for (let errorgroup of ERRORs_array) { 
                ...
                error_details.push({ text: errorgroup[1], nr: errorgroup.length-3, details: details_array });
            }  
        }
        ...
        if (WARNING.length>2) { // if this array contains more than 2 lines "WARNING BEGIN" and "WARNING END"   
            ...
            for (let WARNINGgroup of WARNINGs_array) { 
                ...
                //warning_details.push({ text: WARNINGgroup[1], nr_WARNINGs: WARNINGgroup.length-3, details: details_array });
            }  
        }
    ...
    try {  
        ...
        if (ERROR.length>0) {
            ...
            for (let errorgroup of ERRORs_array) { 
                ...
                error_details.push({ text: errorgroup[1], nr: errorgroup.length-2, details: details_array });
            }   
        } 
